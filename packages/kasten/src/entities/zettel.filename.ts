...
export function generateRandomZettelFileName(extension = ""): ZettelFileName {
  ...
  const fileName = appendExtension(randomWords.join("-"), extension);
  return ZettelFileNameSchema.parse(fileName);
}

function appendExtension(s: string, extension: string): string {
  if (extension === "") {
    return s;
  } else {
    return extension.startsWith(".") ? s + extension : s + "." + extension;
  }
}

// These words come from the English BIP39 list.
const WORDS = [
  "abandon",
  "ability",
  "able",
